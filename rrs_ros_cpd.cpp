...
namespace lmt
{
   ...
   char *argv[])
  {
    ...
    ROS_INFO_STREAM("Franka Mode : " << m_settings.franka_mode);
    ...
    subscriber_nmpc_marker->delegateNewData = std::bind(&Net2TestROSCPD::callbackDataNMPCMarker, this, std::placeholders::_1,std::placeholders::_2,std::placeholders::_3);
    ...
    subscriber_joint_state_franka->delegateNewData = std::bind(&Net2TestROSCPD::callbackDataJointStateFranka, this, std::placeholders::_1,std::placeholders::_2,std::placeholders::_3);
    ...
    publisher_joint_status_franka = net2->publisher("joint_status_franka");
    publisher_joint_status_franka->Start();

    publisher_franka_mode = net2->publisher("franka_mode");
    publisher_franka_mode->Start();

    publisher_eef_franka = net2->publisher("eef_franka");
    publisher_eef_franka->Start();
    ...
    pub_franka_eef = nh.advertise<geometry_msgs::Pose>("/nmpc_controller/out/eef_pose", 1);
    ...
    sub_franka_vel = nh.subscribe("/franka_ros_interface/motion_controller/arm/joint_commands",1, &Net2TestROSCPD::chatterCallbackVelFranka, this);
    sub_franka_eef = nh.subscribe("/nmpc_controller/out/eef_pose",1, &Net2TestROSCPD::chatterCallbackEEFFranka, this);
    sub_franka_status = nh.subscribe("/franka_ros_interface/custom_franka_state_controller/joint_states",1, &Net2TestROSCPD::chatterCallbackStatusFranka, this);
...
void Net2TestROSCPD::receiveCallbackCPD(char * data,int size,int index)
{
  ...
  {
      ROS_WARN("Rigid mode");
      this->rigid.scale(true);
      //this->rigid.reflections(true);
      ...
      ROS_INFO_STREAM("CPD Result Ready " << result.points.rows() << " " << result.scale);
      ...
      else if ( cpd_command.mode() == 2)
  {
      ROS_WARN("Rigid mode no scale");
      this->rigid.scale(false);
      ...
      //We have the trasnform now lets apply it to the main skill
      //result.points = matrixc * result.transform.transpose() + result.translation.transpose().replicate(matrixc.rows(), 1);
      result.points =  matrixc * result.rotation.transpose() + result.translation.transpose().replicate(matrixc.rows(), 1);
...
void Net2TestROSCPD::chatterCallbackVelFranka(const franka_core_msgs::JointCommand::ConstPtr& msg)
{
...
void Net2TestROSCPD::chatterCallbackStatusFranka(const sensor_msgs::JointState msg)
{ 
  //This is the real robot status
  if ( m_settings.franka_mode == "real")
  {
      //We should visualize the robot state in Unity!
      RRSJointState cmd;
      
      //velocity mode
      cmd.add_position(msg.position[0]);
      cmd.add_position(msg.position[1]);
      cmd.add_position(msg.position[2]);
      cmd.add_position(msg.position[3]);
      cmd.add_position(msg.position[4]);
      cmd.add_position(msg.position[5]);
      cmd.add_position(msg.position[6]);
      ...
      //ROS_WARN("Send Done");
      publisher_joint_status_franka->send(buffer,bsize,1);
  }
}

void Net2TestROSCPD::chatterCallbackEEFFranka(const geometry_msgs::Pose msg)
{
 //ROS_INFO("Got velocity for right hand");
 std::cout<<msg.position.x<<msg.position.y<<msg.position.z<<std::endl;
  ...
  cmd.add_goal(msg.position.x);
  cmd.add_goal(msg.position.y);
  cmd.add_goal(msg.position.z);
  cmd.add_goal(msg.orientation.w);
  cmd.add_goal(msg.orientation.x);
  cmd.add_goal(msg.orientation.y);
  cmd.add_goal(msg.orientation.z);
  ...
  //ROS_WARN("Send Done");
  publisher_eef_franka->send(buffer,bsize,1);
}
  ...
  bool Net2TestROSCPD::loadYaml()
  {
    try
    {
      if ( is_file_exist(config_path.c_str()) )
      {
       ...
       m_settings.franka_mode = m_config["franka_mode"].as<std::string>();
...
void Net2TestROSCPD::publishJointStateFranka(char* data, int size)
{
...
std::vector<char> Net2TestROSCPD::callbackDataNMPCMarker(std::vector<char> buffer, unsigned int priority, std::string sender)
{
  std::vector<char> result;
  if ( priority == 10 ) return result;
...
std::vector<char> Net2TestROSCPD::callbackDataJointStateFranka(std::vector<char> buffer, unsigned int priority, std::string sender)
{
  //11
  std::vector<char> result;
  if ( priority == 10 ) return result;

  if ( m_settings.franka_mode == "sim")
...
void Net2TestROSCPD::update()
{
   ...
   kv.set_key("franka_mode");
   kv.set_value(m_settings.franka_mode);

   int bsize = kv.ByteSize();
   char buffer[bsize];
   kv.SerializeToArray(buffer,bsize);

   //ROS_WARN("Send Done");
   publisher_franka_mode->send(buffer,bsize,1);
}
